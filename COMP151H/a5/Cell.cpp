...
{
  throw runtime_error("Can't get int since it is not a int cell!");
}

// check whether it's a DoubleCell(error: if not)
double Cell::get_double() const
{
  throw runtime_error("Can't get double since it is not a double cell!");
}

// check whether it's a SymbolCell(error: if not) 
string Cell::get_symbol() const
{
  throw runtime_error("Can't get symbol since it is not a symbol cell!");
}

// check whether it's a ConsCell(error: if not)
Cell* Cell::get_car() const
{
  throw runtime_error("Can't get car since it is not a cons cell!");
}

// check whether it's a ConsCell(error: if not)
Cell* Cell::get_cdr() const
{
  throw runtime_error("Can't get cdr since it is not a cons cell!");
}

// check whether it's a ProcedureCell(error: if not)
Cell* Cell::get_formals() const
{
  throw runtime_error("Can't get formals since it is not a procedure cell");
}

// check whether it's a ProcedureCell(error: if not)
Cell* Cell::get_body() const
{
  throw runtime_error("Can't get body since it is not a procedure cell");
}
...
{
  throw runtime_error("Only Symbol can be defined others can't");
}
...
{  
  if (my_body == nil || !my_body->is_cons())
      ...
      if (my_formals == nil)
	{
      ...
      // the formal for funciton is a symbol
      else if (my_formals->is_symbol())
	{
	  formals_m = my_formals;
	  body_m = my_body;
	}
      
      // the formals for function is a symbol list
      else if (my_formals->is_cons())
	{
	  ...
	  while (temp_ptr != nil)
		...
		{
		  throw runtime_error("The formals for funtion should be symbol or symbol list");
		}
      ...
      // throw error if the formals are illegal
      else
	{
	  throw runtime_error("The formals for function should be symbol or a symbol list");
	}
    }
}
